...
> **PHASE-3 INSERTION NOTE (2026-05-09)**
>
> This document was authored as a quick task (`260509-fox`) and is now part of Phase 3's planning bundle. It supplements `03-CONTEXT.md`; it does **not** replace it.
>
> **Coordinate reconciliation required.** The prop coordinates picked downstream of this context were marked "Claude's Discretion" — they were NOT user-locked. They conflict with shipped Phase 2 positions referenced in `03-CONTEXT.md`:
>
> | Prop       | Scenario plan picks         | Shipped Phase 2 (authoritative)        |
> |------------|-----------------------------|----------------------------------------|
> | BTR/target | `(-15, 0, 0)`               | `TARGET_WORLD = (100, 1, 200)`         |
> | Friendlies | `(60, 1.5, 30)`             | `FRIENDLIES_WORLD = (100, 2, 1000)`    |
> | Camera     | `(0, 35, 250)`              | existing Phase 2 vantage               |
>
> When executing `03-SCENARIO-PLAN.md`, the executor MUST reconcile to the shipped values from `src/lib/positions.ts` (introduced in Phase 3 per `03-CONTEXT.md`) and recompute slant range, friendlies-distance, canonical MGRS (`345678` likely needs to change), and digit-misread fallback against the actual `worldToGrid` output. The locked DECISIONS are the **demo flow** (two-take wrong-grid punchline) and **HUD behavior** (live grid + lase locks range), not the geometry.
